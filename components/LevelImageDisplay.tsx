...
function fixUnfilledStrokeForDarkTheme(node: React.ReactNode): React.ReactNode {
  ...
  // <Line> has no fill concept at all (never set in JSX), so it wouldn't
  // match `fill === 'none'` even though it's just as "unfilled" as a
  // fill="none" Circle/Path — both need the same dark-theme fallback.
  const isUnfilled = props.fill === 'none' || node.type === Line;
  if (isUnfilled && (props.stroke === '#000000' || props.stroke === 'black')) {
