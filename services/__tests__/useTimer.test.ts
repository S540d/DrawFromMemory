...
describe('useTimer', () => {
  ...
  it('starts with timeRemaining = 0', () => {
    const { result } = renderHook(() => useTimer({ phase: 'memorize', onExpire: jest.fn() }));
  ...
  it('counts down every second during memorize phase', () => {
    ...
    act(() => {
      result.current.setTimeRemaining(3);
    });
  ...
  it('calls onExpire when countdown reaches 0', () => {
    const onExpire = jest.fn();
    const { result } = renderHook(() => useTimer({ phase: 'memorize', onExpire }));
  ...
  it('does not count down during draw phase', () => {
    const { result } = renderHook(() => useTimer({ phase: 'draw', onExpire: jest.fn() }));

    act(() => {
      result.current.setTimeRemaining(5);
    });
    act(() => {
  ...
  it('does not count down during result phase', () => {
    const { result } = renderHook(() => useTimer({ phase: 'result', onExpire: jest.fn() }));
